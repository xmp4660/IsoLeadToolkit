...
class ControlPanel:
    ...
    def _build_geo_tab(self, parent):
        """Build the Geochemistry Parameters tab with optimize layout (Grid)"""
        ...
        # self.geo_vars is initialized in __init__ to support shared access
        ...
        # Subheaders as grid items spanning 2 columns
        ...
        # Algorithm Presets logic could go here
        ...
        def apply_changes():
            ...
            try:
                ...
                if app_state.df_global is not None:
                    # Maybe re-run export logic dry run or just visual update?
                    # Currently V1V2 calculation happens on export or diagram. 
                    # If we have V1V2 diagram active, refresh it.
                    if app_state.render_mode == 'V1V2':
                        self._on_change() # Triggers re-render
        ...
        def reset_defaults():
            # Defaults logic: Reload module or Hardcode defaults?
            # Hardcoding SK defaults here for safety
            defaults = {
                'Tsec': 3700,
    ...
    def _on_model_selected(self, event=None):
        ...
        if not model_name or geochemistry is None: return
    ...
    def _build_algorithm_tab(self, parent):
        ...
        def add_v1v2_entry(parent_frame, label_key, default_val, var_key):
            ...
            var = tk.StringVar(value=str(default_val))
            self.geo_vars[var_key] = var # Use geo_vars so apply_changes still works vaguely, or handle manually
        ...
        # We need to access current Gemoetry params (T1, T2)
        try:
            current_params = geochemistry.engine.get_parameters()
        except AttributeError:
             current_params = {}
        ...
        # Apply Logic for V1V2 params
        # Note: We bind this to entry change.
    ...
    def _on_v1v2_param_change(self):
        ...
        try:
            ...
            if new_params and geochemistry:
                geochemistry.engine.update_parameters(new_params)
    ...
    def _update_algorithm_visibility(self):
        ...
        for section in ['umap_section', 'tsne_section', 'pca_section', 'rpca_section', 'ternary_section', 'v1v2_section', 'isochron_section', 'twod_section']:
