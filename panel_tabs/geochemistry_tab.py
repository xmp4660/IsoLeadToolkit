...
class GeochemistryTabMixin:
    ...
    def _build_geo_tab(self, parent):
        ...
        if geochemistry is None:
            lbl = ttk.Label(
                frame,
                text=self._translate("Geochemical module not loaded."),
        ...
        # 0. 模型选择：预设参数组合（如 Stacey-Kramers 等）。
        ...
        # 3. 初始铅同位素组成：
        #    a0/b0/c0 为原始端元（T1/T2），a1/b1/c1 为两阶段模型第二阶段端元。
        ...
        # 4. 地幔与产率相关参数：
        #    μ/ω 反映 U/Pb 与 Th/U 的有效比值，U_ratio 为 235U/238U 比值。
        ...
        def get_float(var_key):
            value = self.geo_vars[var_key].get()
            return float(value)
        ...
        def apply_changes():
            ...
            try:
                # 计算原理：
                # - 时间参数以 Ma 输入，乘 1e6 转为年，保持与模型方程一致。
                # - 其余参数直接按浮点数写入，用于同位素演化方程计算。
                if 'T1' in self.geo_vars:
                    new_params['T1'] = get_float('T1') * 1e6
                if 'T2' in self.geo_vars:
                    new_params['T2'] = get_float('T2') * 1e6
                if 'Tsec' in self.geo_vars:
                    new_params['Tsec'] = get_float('Tsec') * 1e6

                # 其余参数直接转换为浮点数。
                direct_keys = (
                    'lambda_238', 'lambda_235', 'lambda_232',
                    'a0', 'b0', 'c0',
                    'a1', 'b1', 'c1',
                ...
                messagebox.showinfo(
                    ...
                    self._translate("Parameters updated successfully."),
                )
