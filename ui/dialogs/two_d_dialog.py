...
class _Select2DColumnsDialog:
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            container,
            text=translate("Choose axes for the 2D scatter"),
            style='Header.TLabel',
            wraplength=340,
            justify=tk.LEFT
        )
    ...
    def _on_ok(self):
        ...
        if len(set(selections)) != 2:
            ...
            return
        try:
            print(f"[DEBUG] 2D dialog confirmed selection: {selections}, KDE: {self.kde_var.get()}", flush=True)
        except Exception:
            pass
        self.result = (selections, self.kde_var.get())
        self._close()
